...
#include "/usr/local/include/opencv4/opencv2/core/utility.hpp"
#include "/usr/local/include/opencv4/opencv2/text.hpp"
...
#include "mm_ocr/OCR_message.h"
#include "mm_ocr/SPR_message.h"
...
void confirmStartOcrCallback(const mm_ocr::SPR_message::ConstPtr& msg);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_OcrWait = nodeHandler1.subscribe("SPR_OCR_Channel", 100, confirmStartOcrCallback);
    ros::spin();
    return 0;
}
...
void confirmStartOcrCallback(const mm_ocr::SPR_message::ConstPtr& msg)
{
    ...
    std::stringstream ss;
    mm_ocr::OCR_message ocr_msg;
    mm_ocr::SPR_message checkMessage;

    ros::NodeHandle nodeHandler2;
    // Check the subscribed topic string, if confirmation exists then enter the OCR routine
    std::string checkString = static_cast<std::string>(msg->Scan.c_str());
    std::cout << checkMessage << std::endl;

    // Start a publisher
    ros::Publisher pub_OcrExecute = nodeHandler2.advertise<mm_ocr::OCR_message>("OCR_DB_Channel", 100);
...
std::map<int,std::string> performOcr()
{
    String model = static_cast<String>("/home/george/ros_ws/src/mm_ocr/src/frozen_east_text_detection.pb");
    Net net;
